...
impl TextArea {
    ...
    #[cfg(test)]
    fn set_cursor_for_test(&mut self, row: usize, col: usize) {
...
/// Saturating cast — saturates rather than panicking; the surrounding Rect
/// already bounds inputs to u16 in practice.
...
mod tests {
    ...
    // Pin char-vs-byte translation for multi-byte input. Each codepoint
    // counts as one cursor column; Backspace removes one whole codepoint.
    #[test]
    fn non_ascii_inserts_advance_cursor_one_per_char() {
        ...
        // Backspace in a single-char multi-byte buffer must clear it cleanly
        // (regression guard for char_offset_to_byte arithmetic).
        let mut ta2 = TextArea::default();
        ta2.insert_str("é");
    ...
    // Regression guard: with an out-of-range cursor, Backspace previously
    // claimed modification when no character was removed. The private cursor
    // field plus clamping in set_cursor_for_test prevent that.
    #[test]
    fn out_of_range_cursor_is_clamped_and_backspace_is_correct() {
        ...
        assert_eq!(ta.cursor, (0, 3));
        let modified = ta.input(key(KeyCode::Backspace));
        // One character actually removed.
        assert!(modified);
        assert_eq!(ta.lines(), &["ab".to_owned()]);
        assert_eq!(ta.cursor, (0, 2));
    }
    ...
    fn render_places_each_line_on_its_own_row() {
        ...
        let mut ta = TextArea::default();
        ta.insert_str("aa\nbb\ncc");
        // Cursor parks on row 3 (an unrendered empty row past the content)
        // so every probed cell on rows 0-2 is non-cursor and unstyled.
