...
- Revset resolution via jj log: roots(trunk()..@)..@ for --stack, arbitrary revsets for jjr <revset>, falling back to @ with a stderr warning if trunk() is unresolvable or empty
...
- jjr --stack resolves roots(trunk()..@)..@ and walks oldest-to-newest
...
- Main view footer drops bindings right-to-left at narrow widths, preserving Enter comment / Tab file / n/p change / ↑↓ line
