//! Opaque identifiers for changes and commits.
//!
//! `ChangeId` models a jj change ID (e.g. `abc12345` or `abc12345/1` for
//! divergent changes). `CommitId` models a hex commit SHA. Both are validated
//! at the parse boundary; downstream code can trust the values.
...
/// optional `/<divergence>`) is also kept so [`as_str`](ChangeId::as_str) and
/// the transparent serde wire format remain a borrowed `&str`.
///
...
impl CommitId {
    pub fn parse(raw: &str) -> Result<Self> {
        ...
        if !is_valid_commit_id(trimmed) {
            return Err(Error::InvalidCommitId {
                raw: raw.to_owned(),
            });
        }
